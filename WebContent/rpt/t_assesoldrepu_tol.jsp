...
var storeRepoPeriodUnit=Ext.create('Ext.data.Store', { fields: kValFields, data : <%=DicUtil.getInstance().getRptDic("T_Repo_Period_Unit_Cde") %>});
var getRepoPeriodUnitRender=function(value){var r = storeRepoPeriodUnit.getById(value);if (!Ext.isEmpty(r)) {return r.data.name;}else{return value;}};
...
<script type="text/javascript" src="<%=request.getContextPath()%>/resource/view/rpt/t_assesoldrepu_tol/TAssesoldrepuTollist.js?_t=<%=Math.random()%>"></script>
<script type="text/javascript" src="<%=request.getContextPath()%>/resource/view/rpt/t_assesoldrepu_tol/TAssesoldrepuToledit.js?_t=<%=Math.random()%>"></script>
</head>
<body>
<script type="text/javascript">	
	    Ext.onReady(InitTAssesoldrepuTolList);
</script>
</body>
</html>
