...
var storeOppositeCustAttri=Ext.create('Ext.data.Store', { fields: kValFields, data : <%=DicUtil.getInstance().getRptDic("T_Opp_Cust_Attri_Cde") %>});
var getOppositeCustAttriRender=function(value){var r = storeOppositeCustAttri.getById(value);if (!Ext.isEmpty(r)) {return r.data.name;}else{return value;}};
...
//var storePeriodDic=Ext.create('Ext.data.Store', { fields: kValFields, data : <%=DicUtil.getInstance().getRptDic("T_Period_Unit_Cde") %>});
//var getTxnAttriRender=function(value){var r = storePeriodDic.getById(value);if (!Ext.isEmpty(r)) {return r.data.name;}else{return value;}};
...
var storeSaleAssetType=Ext.create('Ext.data.Store', { fields: kValFields, data : <%=DicUtil.getInstance().getRptDic("T_Sell_Asset_Cat_Cde") %>});
var getSaleAssetTypeRender=function(value){var r = storeSaleAssetType.getById(value);if (!Ext.isEmpty(r)) {return r.data.name;}else{return value;}};
...
<script type="text/javascript" src="<%=request.getContextPath()%>/resource/view/rpt/t_buybackassetacquired_dtl/TBuybackassetacquiredDtllist.js?_t=<%=Math.random()%>"></script>
<script type="text/javascript" src="<%=request.getContextPath()%>/resource/view/rpt/t_buybackassetacquired_dtl/TBuybackassetacquiredDtledit.js?_t=<%=Math.random()%>"></script>
</head>
<body>
<script type="text/javascript">	
	    Ext.onReady(InitTBuybackassetacquiredDtlList);
</script>
</body>
</html>
