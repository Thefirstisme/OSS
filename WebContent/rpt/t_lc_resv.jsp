...
var storePeriodCon=Ext.create('Ext.data.Store', { fields: kValFields, data : <%=DicUtil.getInstance().getRptDic("T_Period_Con_Cde") %>});
var getPeriodConRender=function(value){var r = storePeriodCon.getById(value);if (!Ext.isEmpty(r)) {return r.data.name;}else{return value;}};
...
<script type="text/javascript" src="<%=request.getContextPath()%>/resource/view/rpt/t_lc_resv/TLcResvlist.js?_t=<%=Math.random()%>"></script>
<script type="text/javascript" src="<%=request.getContextPath()%>/resource/view/rpt/t_lc_resv/TLcResvedit.js?_t=<%=Math.random()%>"></script>
</head>
<body>
<script type="text/javascript">	
	    Ext.onReady(InitTLcResvList);
</script>
</body>
</html>
