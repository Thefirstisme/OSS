...
var storeCatCd=Ext.create('Ext.data.Store', { fields: kValFields, data : <%=DicUtil.getInstance().getRptDic("T_Loan_Cat_Cde") %>});
var getCatCdRender=function(value){var r = storeCatCd.getById(value);if (!Ext.isEmpty(r)) {return r.data.name;}else{return value;}};

var storeCurrencyCd1=Ext.create('Ext.data.Store', { fields: kValFields, data : <%=DicUtil.getInstance().getRptDic("T_Currency_Cde") %>});
var getCurrencyCdRender=function(value){var r = storeCurrencyCd1.getById(value);if (!Ext.isEmpty(r)) {return r.data.name;}else{return value;}};
...
var storeOverdue=Ext.create('Ext.data.Store', { fields: kValFields, data : <%=DicUtil.getInstance().getRptDic("T_Loan_Stat_Cde") %>});
var getOverdueRender=function(value){var r = storeOverdue.getById(value);if (!Ext.isEmpty(r)) {return r.data.name;}else{return value;}};
...
<script type="text/javascript" src="<%=request.getContextPath()%>/resource/view/rpt/t_totaloan_tol/TTotaloanTollist.js?_t=<%=Math.random()%>"></script>
<script type="text/javascript" src="<%=request.getContextPath()%>/resource/view/rpt/t_totaloan_tol/TTotaloanToledit.js?_t=<%=Math.random()%>"></script>
</head>
<body>
<script type="text/javascript">	
	    Ext.onReady(InitTTotaloanTolList);
</script>
</body>
</html>
