...
<title>外汇买卖（即期）-汇总</title>
<script type="text/javascript">

var storeFxTradeType=Ext.create('Ext.data.Store', { fields: kValFields, data : <%=DicUtil.getInstance().getRptDic("T_Fx_Trade_Type_Cde") %>});
var getFxTradeTypeRender=function(value){var r = storeFxTradeType.getById(value);if (!Ext.isEmpty(r)) {return r.data.name;}else{return value;}};
...
<script type="text/javascript" src="<%=request.getContextPath()%>/resource/view/rpt/t_forextradespot_sour_tol/TForextradespotSourTollist.js?_t=<%=Math.random()%>"></script>
<script type="text/javascript" src="<%=request.getContextPath()%>/resource/view/rpt/t_forextradespot_sour_tol/TForextradespotSourToledit.js?_t=<%=Math.random()%>"></script>
</head>
<body>
<script type="text/javascript">	
	    Ext.onReady(InitTForextradespotSourTolList);
</script>
</body>
</html>
