 ...
 * 
 * <p>
 * If using the JobInitializationPlugin with JobStoreCMT, be sure to set the plugin property <em>wrapInUserTransaction</em> to true. Also, if have a
 * positive <em>scanInterval</em> be sure to set <em>org.quartz.scheduler.wrapJobExecutionInUserTransaction</em> to true.
 * </p>
 * 
 * @author James House
...
public class JobInitializationPlugin extends SchedulerPluginWithUserTransactionSupport implements FileScanListener {
            ...
            if (jobFiles.isEmpty() == false) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (scanInterval > 0) {
                        ...
                        SimpleTrigger trig = new SimpleTrigger(jobTriggerName, JOB_INITIALIZATION_PLUGIN_NAME, new Date(), null,
    ...
    /**
     * Helper method for generating unique job/trigger name for the file scanning jobs (one per FileJob). The unique names are saved in
     * jobTriggerNameSet.
     */
    ...
    private void processFile(JobFile jobFile) {
        ...
        try {
            processor.processFileAndScheduleJobs(jobFile.getFileName(), jobFile.getFileName(), // systemId
                    getScheduler(), isOverWriteExistingJobs());
        } catch (Exception e) {
    ...
    public void processFile(String fileName) {
        ...
        furl = (furl != null) ? furl : file.getAbsolutePath();
        getLog().info("Correct file path:" + furl);
        processFile((JobFile) jobFiles.get(furl));
        // === correct file path===
