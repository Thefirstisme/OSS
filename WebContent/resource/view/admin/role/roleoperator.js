Ext.define('OSS.view.RoleOperatorRel', {
...
var InitRoleOperatorRel=function(){
	Ext.tip.QuickTipManager.init();
	new OSS.view.RoleOperatorRel();
};
...
var setTreeGridInit=function(){
	...
	var myMask = new Ext.LoadMask(tree.body, { msg: '正在加载【'+roleText+'】权限,请稍等...' });
    myMask.show();
	tStore.load(
			{
				params:{id:roleId},
				//waitMsg : '正在加载【'+roleText+'】对应权限数据，请稍后...' 
...
var dataSubmit=function(){
	...
	if(permitId.length>0){
		...
		Ext.Ajax.request({ 
			url: "/OSS/admin/source/source_setRolePermit",
			method : 'post', 
			params : {id: permitId},
			success : function(response, options){ 
				 var data = Ext.JSON.decode(response.responseText);   
		...
		Ext.MessageBox.show({title : '事件信息',msg :'权限未发生变化！',buttons: Ext.MessageBox.OK, icon: Ext.MessageBox.WARNING});
	}
};
