Ext.define('OSS.view.TSecuritieTolList', {
	...
	onShbRHClick : function(item, e, eOpts) {
		...
		OSS.setExtValue('tableName', 'T_Securitie_Tol');
		OSS.setExtValue('tPath', 'TSecuritieTol');
	},
...
//		window.parent.ContentTablePanelSet('5f5134623239493f84022c11ee5e4c3c',
//				'有价证券-汇总明细', 'rpt/t_securitie_dtl.jsp?mainId='
...
//  				.getSelection();
  		...
  		window.location.href='t_securitie_dtl.jsp?mainId='+OSS.getExtValue('hidMainId')+ '&'+OSS.paraName+'='+OSS.userId;
  	},   
...
var storeMain = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName + '/rpt/rptTSecuritieTol_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName + '/rpt/rptTSecuritieDtl_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
				url : OSS.getAppName + "/rpt/rptTSecuritieTol_toDel",
				method : 'post',
				params : {
					id : id
				},
...
var InitTSecuritieTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TSecuritieTolList();
};
