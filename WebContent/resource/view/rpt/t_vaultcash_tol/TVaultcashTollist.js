Ext.define('OSS.view.TVaultcashTolList', {
	...
	onShbRHClick : function(item, e, eOpts) {
		...
		OSS.setExtValue('tableName', 'T_VaultCash_Tol');
		OSS.setExtValue('tPath', 'TVaultcashTol');
	},
...
//		window.parent.ContentTablePanelSet('0d0902c715864e9ba0f48fb447cae70f',
//				'库存现金-汇总明细', 'rpt/t_vaultcash_dtl.jsp?mainId='
...
//  				.getSelection();
  		...
  		window.location.href='t_vaultcash_dtl.jsp?mainId='+OSS.getExtValue('hidMainId')+ '&'+OSS.paraName+'='+OSS.userId;
  	},   
...
var storeMain = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName + '/rpt/rptTVaultcashTol_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName + '/rpt/rptTVaultcashDtl_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
				url : OSS.getAppName + "/rpt/rptTVaultcashTol_toDel",
				method : 'post',
				params : {
					id : id
				},
...
var InitTVaultcashTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TVaultcashTolList();
};
