Ext
		.define(
				'OSS.view.TAccopayatempTolList',
				{
					...
					// window.parent.ContentTablePanelSet('f5e25fa541e1486a9ac250eae367ae9e',
					// '应付及暂收款-汇总明细', 'rpt/t_accopayatemp_dtl.jsp?mainId='
					// + OSS.getExtValue('hidMainId'));
					// }
					// ,
					infoListMore : function(item, e, eOpts) {
						var fixDepoInd = Ext.getCmp('GridPanelMain')
						...
						window.location.href = 't_accopayatemp_dtl.jsp?mainId='
								+ OSS.getExtValue('hidMainId') + '&fixDepoInd='
								+ fixDepoInd[0].get("fixDepoInd")+'&'+OSS.paraName+'='+OSS.userId;
						;
					},
					...
					onShbRHClick : function(item, e, eOpts) {
						...
						OSS.setExtValue('tableName', 'T_AccoPayaTemp_Tol');
						OSS.setExtValue('tPath', 'TAccopayatempTol');
					},
...
var storeMain = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTAccopayatempTol_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTAccopayatempDtl_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
		url : OSS.getAppName + "/rpt/rptTAccopayatempTol_toDel",
		method : 'post',
		params : {
			id : id
		},
...
var InitTAccopayatempTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TAccopayatempTolList();
};
