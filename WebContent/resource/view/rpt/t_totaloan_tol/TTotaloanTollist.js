		...
		.define(
				'OSS.view.TTotaloanTolList',
				{
					...
					onShbRHClick : function(item, e, eOpts) {
						...
						OSS.setExtValue('tableName', 'T_TotaLoan_Tol');
						OSS.setExtValue('tPath', 'TTotaloanTol');
					},
					...
					// window.parent.ContentTablePanelSet('7dee6e299294443f86c0859c618c97f2',
					// '各项贷款-汇总明细', 'rpt/t_totaloan_dtl.jsp?mainId='
					...
					infoListMore : function(item, e, eOpts) {
						...
						window.location.href = 't_totaloan_dtl.jsp?mainId='
...
var storeMain = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTTotaloanTol_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTTotaloanDtl_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
		url : OSS.getAppName + "/rpt/rptTTotaloanTol_toDel",
		method : 'post',
		params : {
			id : id
		},
...
var InitTTotaloanTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TTotaloanTolList();
};
