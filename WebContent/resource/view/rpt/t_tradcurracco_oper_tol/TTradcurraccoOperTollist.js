Ext.define('OSS.view.TTradcurraccoOperTolList', {
	...
	onShbRHClick : function(item, e, eOpts) {
		...
		OSS.setExtValue('tableName', 'T_TradCurrAcco_Oper_Tol');
		OSS.setExtValue('tPath', 'TTradcurraccoOperTol');
	},
...
//		window.parent.ContentTablePanelSet('8287aaada8a343d2b0ca268e6ed70427',
//				'同业往来（运用方）-汇总明细', 'rpt/t_tradcurracco_oper_dtl.jsp?mainId='
...
//  				.getSelection();
  		...
  		window.location.href='t_tradcurracco_oper_dtl.jsp?mainId='+OSS.getExtValue('hidMainId')+ '&'+OSS.paraName+'='+OSS.userId;
  	},   
...
var storeMain = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName
								+ '/rpt/rptTTradcurraccoOperTol_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName
								+ '/rpt/rptTTradcurraccoOperDtl_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
				url : OSS.getAppName + "/rpt/rptTTradcurraccoOperTol_toDel",
				method : 'post',
				params : {
					id : id
				},
...
var InitTTradcurraccoOperTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TTradcurraccoOperTolList();
};
