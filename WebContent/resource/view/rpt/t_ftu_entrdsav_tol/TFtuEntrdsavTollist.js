Ext.define('OSS.view.TFtuEntrdsavTolList', {
	...
	onShbRHClick : function(item, e, eOpts) {
		...
		OSS.setExtValue('tableName', 'T_FTU_EntrdSav_Tol');
		OSS.setExtValue('tPath', 'TFtuEntrdsavTol');
	},
...
//		window.parent.ContentTablePanelSet('db468322adb247a68787664e5d846545',
//				'FTU委托存款及委托投资基金（净）-汇总明细', 'rpt/t_ftu_entrdsav_dtl.jsp?mainId='
...
//  				.getSelection();
  		...
  		window.location.href='t_ftu_entrdsav_dtl.jsp?mainId='+OSS.getExtValue('hidMainId')+ '&'+OSS.paraName+'='+OSS.userId;
  	},   
...
var storeMain = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName + '/rpt/rptTFtuEntrdsavTol_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
				url : OSS.getAppName + "/rpt/rptTFtuEntrdsavTol_toDel",
				method : 'post',
				params : {
					id : id
				},
...
var InitTFtuEntrdsavTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TFtuEntrdsavTolList();
};
