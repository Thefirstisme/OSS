Ext
		.define(
				'OSS.view.TSysfinaflowSourTolList',
				{
					...
					onShbRHClick : function(item, e, eOpts) {
						...
						OSS.setExtValue('tableName', 'T_SysFinaFlow_Sour_Tol');
						OSS.setExtValue('tPath', 'TSysfinaflowSourTol');
					},
					...
					// window.parent.ContentTablePanelSet('62a8fbc3a9dd4b4d879baafc753628d1',
					// '系统内资金往来（来源方）-汇总明细',
					// 'rpt/t_sysfinaflow_sour_dtl.jsp?mainId='
					...
					infoListMore : function(item, e, eOpts) {
						...
						window.location.href = 't_sysfinaflow_sour_dtl.jsp?mainId='
...
var storeMain = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTSysfinaflowSourTol_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTSysfinaflowSourDtl_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
		url : OSS.getAppName + "/rpt/rptTSysfinaflowSourTol_toDel",
		method : 'post',
		params : {
			id : id
		},
...
var InitTSysfinaflowSourTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TSysfinaflowSourTolList();
};
