Ext.define('OSS.view.TFtuMedlongborrTolList', {
	...
	onShbRHClick : function(item, e, eOpts) {
		...
		OSS.setExtValue('tableName', 'T_FTU_MedLongBorr_Tol');
		OSS.setExtValue('tPath', 'TFtuMedlongborrTol');
	},
...
//		window.parent.ContentTablePanelSet('61e5e7c268834a219b97aa8b6dc60a5d',
//				'FTU内部活动-中长期借款-汇总明细', 'rpt/t_ftu_medlongborr_dtl.jsp?mainId='
...
//  				.getSelection();
  		...
  		window.location.href='t_ftu_medlongborr_dtl.jsp?mainId='+OSS.getExtValue('hidMainId')+ '&'+OSS.paraName+'='+OSS.userId;
  	},   
...
var storeMain = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName
								+ '/rpt/rptTFtuMedlongborrTol_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName
								+ '/rpt/rptTFtuMedlongborrDtl_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
				url : OSS.getAppName + "/rpt/rptTFtuMedlongborrTol_toDel",
				method : 'post',
				params : {
					id : id
				},
...
var InitTFtuMedlongborrTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TFtuMedlongborrTolList();
};
