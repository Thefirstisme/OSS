Ext
		.define(
				'OSS.view.TTradcurraccoSourTolList',
				{
					...
					onShbRHClick : function(item, e, eOpts) {
						...
						OSS.setExtValue('tableName', 'T_TradCurrAcco_Sour_Tol');
						OSS.setExtValue('tPath', 'TTradcurraccoSourTol');
					},
					...
					// window.parent.ContentTablePanelSet('37981842400147859dcde6a03dccd4ff',
					// '同业往来（来源方）-汇总明细',
					// 'rpt/t_tradcurracco_sour_dtl.jsp?mainId='
					...
					infoListMore : function(item, e, eOpts) {
						...
						window.location.href = 't_tradcurracco_sour_dtl.jsp?mainId='
...
var storeMain = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTTradcurraccoSourTol_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTTradcurraccoSourDtl_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
		url : OSS.getAppName + "/rpt/rptTTradcurraccoSourTol_toDel",
		method : 'post',
		params : {
			id : id
		},
...
var InitTTradcurraccoSourTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TTradcurraccoSourTolList();
};
