Ext.define('OSS.view.TFtuFinabondTolList', {
	...
	onShbRHClick : function(item, e, eOpts) {
		...
		OSS.setExtValue('tableName', 'T_FTU_FinaBond_Tol');
		OSS.setExtValue('tPath', 'TFtuFinabondTol');
	},
...
//		window.parent.ContentTablePanelSet('c34d23212da5423daec0dd575d15c202',
//				'FTU内部活动-金融债券-汇总明细', 'rpt/t_ftu_finabond_dtl.jsp?mainId='
...
//  				.getSelection();
  		...
  		window.location.href='t_ftu_finabond_dtl.jsp?mainId='+OSS.getExtValue('hidMainId')+ '&'+OSS.paraName+'='+OSS.userId;
  	},   
...
var storeMain = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName + '/rpt/rptTFtuFinabondTol_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName + '/rpt/rptTFtuFinabondDtl_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
				url : OSS.getAppName + "/rpt/rptTFtuFinabondTol_toDel",
				method : 'post',
				params : {
					id : id
				},
...
var InitTFtuFinabondTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TFtuFinabondTolList();
};
