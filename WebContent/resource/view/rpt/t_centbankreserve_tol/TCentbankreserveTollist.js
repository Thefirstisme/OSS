Ext.define('OSS.view.TCentbankreserveTolList', {
	...
	onShbRHClick : function(item, e, eOpts) {
		...
		OSS.setExtValue('tableName', 'T_CentBankReserve_Tol');
		OSS.setExtValue('tPath', 'TCentbankreserveTol');
	},
...
//		window.parent.ContentTablePanelSet('2d837e7051624fc697d632dc9419deec',
//				'存放中央准备金存款-汇总明细', 'rpt/t_centbankreserve_dtl.jsp?mainId='
...
//  				.getSelection();
  		...
  		window.location.href='t_centbankreserve_dtl.jsp?mainId='+OSS.getExtValue('hidMainId')+ '&'+OSS.paraName+'='+OSS.userId;
  	},   
...
var storeMain = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName
								+ '/rpt/rptTCentbankreserveTol_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName
								+ '/rpt/rptTCentbankreserveDtl_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
				url : OSS.getAppName + "/rpt/rptTCentbankreserveTol_toDel",
				method : 'post',
				params : {
					id : id
				},
...
var InitTCentbankreserveTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TCentbankreserveTolList();
};
