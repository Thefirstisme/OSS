...
function getTSecuritieDtlInfo() {
	...
	if (keyid != "0") {
		...
		myMask.show();
		Ext.Ajax.request({
					url : OSS.getAppName + '/rpt/rptTSecuritieDtl_toEnt',
					method : 'post',
					params : {
						id : keyid
					},
					success : function(result) {
						...
						OSS.setExtValue('secuType', d.secuType);
						OSS.setExtValue('securitiescode', d.securitiescode);
...
var toUpdate = function() {
	...
	if (formpPanel1.form.isValid()) {
		...
		Ext.Ajax.request({
					url : OSS.getAppName + "/rpt/rptTSecuritieDtl_" + actionId,
					method : 'post',
					params : params,
