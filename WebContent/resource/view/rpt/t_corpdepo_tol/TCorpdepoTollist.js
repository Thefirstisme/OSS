...
//									},
	...
	infoListMore : function(item, e, eOpts) {
		...
		window.location.href='t_corpdepo_dtl.jsp?mainId='+OSS.getExtValue('hidMainId')+'&fixDepoInd='+fixDepoInd[0].get("fixDepoInd")+ '&'+OSS.paraName+'='+OSS.userId;
	},
...
var storeMain = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName + '/rpt/rptTCorpdepoTol_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
//	 //alert(userId);
//	 OSS.setExtValue('userId', userId);
	 
	if(dt!=undefined && dt.length>0){
		decDt=dt;
	}
			var new_params = {
			    subaccountno : OSS.getExtValue('txtSearchAcctId'),
				declareDt: decDt
				//userId:userId
			};
...
//	var status = JudgeAuthority(id);
//	 alert("status--->"+status);
//	if(status=="1"){
//		alert("数据已上报或在上报中，不可以修改"); 
//	}else if(status=="3"){
//		alert("数据上报中，不可以修改"); 
//    }else{
//	    OSS.setExtValue('status',status);
	...
	//	OSS.setExtValue('useId', OSS.getExtValue('userId')); 
