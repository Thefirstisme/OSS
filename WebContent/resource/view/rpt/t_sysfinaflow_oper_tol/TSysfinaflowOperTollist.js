Ext
		.define(
				'OSS.view.TSysfinaflowOperTolList',
				{
					...
					onShbRHClick : function(item, e, eOpts) {
						...
						OSS.setExtValue('tableName', 'T_SysFinaFlow_Oper_Tol');
						OSS.setExtValue('tPath', 'TSysfinaflowOperTol');
					},
					...
					// window.parent.ContentTablePanelSet('dea00f7a8103477abe1fe99cf60d4d1f',
					// '系统内资金往来（运用方）-汇总明细',
					// 'rpt/t_sysfinaflow_oper_dtl.jsp?mainId='
					...
					infoListMore : function(item, e, eOpts) {
						...
						window.location.href = 't_sysfinaflow_oper_dtl.jsp?mainId='
...
var storeMain = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTSysfinaflowOperTol_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTSysfinaflowOperDtl_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
		url : OSS.getAppName + "/rpt/rptTSysfinaflowOperTol_toDel",
		method : 'post',
		params : {
			id : id
		},
...
var InitTSysfinaflowOperTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TSysfinaflowOperTolList();
};
