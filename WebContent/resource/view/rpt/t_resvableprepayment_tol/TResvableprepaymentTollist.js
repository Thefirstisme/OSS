Ext
		.define(
				'OSS.view.TResvableprepaymentTolList',
				{
					...
					onGridPanelItemDblClick : function(dataview, record, item,
							index, e, eOpts) {
					...
					onShbRHClick : function(item, e, eOpts) {
						...
						OSS
								.setExtValue('tableName',
										'T_ResvablePrepayment_Tol');
						OSS.setExtValue('tPath', 'TResvableprepaymentTol');
					},
					...
					// window.parent.ContentTablePanelSet('2a82299dfb004485aef7a60b85595172',
					// '应收及预付款-汇总明细', 'rpt/t_resvableprepayment_dtl.jsp?mainId='
					...
					infoListMore : function(item, e, eOpts) {
						// var fixDepoInd =
						// Ext.getCmp('GridPanelMain').getSelectionModel()
						// .getSelection();
						// window.parent.ContentTablePanelSet('572e81ba481f4108bd5689cfda99a105','单位存款-汇总明细',
						// 'rpt/t_corpdepo_dtl.jsp?mainId='+
						// OSS.getExtValue('hidMainId') + "&fixDepoInd="+
						// fixDepoInd[0].get("fixDepoInd"));
						window.location.href = 't_resvableprepayment_dtl.jsp?mainId='
								+ OSS.getExtValue('hidMainId')+ '&'+OSS.paraName+'='+OSS.userId;
					},
...
var storeMain = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTResvableprepaymentTol_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTResvableprepaymentDtl_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
logInfoListStore.on('beforeload', function(store, options) {
	var tempMainId = OSS.getExtValue('hidMainId');
	
	var ids = tempMainId.split("_");
...
function editRowData(id, editType) {
	...
	OSS.getExtControl('currencyCd').setDisabled(false);
	OSS.getExtControl('belongOrgId').setDisabled(false);
	...
	if (id != '0') {
		...
		OSS.getExtControl('currencyCd').setDisabled(true);
		OSS.getExtControl('belongOrgId').setDisabled(true);
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
		url : OSS.getAppName + "/rpt/rptTResvableprepaymentTol_toDel",
		method : 'post',
		params : {
			id : id
		},
...
var InitTResvableprepaymentTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TResvableprepaymentTolList();
};
