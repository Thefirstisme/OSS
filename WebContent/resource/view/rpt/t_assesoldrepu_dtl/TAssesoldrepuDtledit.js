					...
					items : [{
								...
								items : [{
										...
										// },
										{
											...
											listeners:{  
											    'change':function(){    
											    	setMatureDt('intStartDt', 'repoPeriodLen','repoPeriodUnit','matureDt');    
											    }
											}
										}, {
											xtype : 'combobox',
...
var initWin;
function getTAssesoldrepuDtlInfo() {
	...
	if (keyid != "0") {
		...
		myMask.show();
		Ext.Ajax.request({
					url : OSS.getAppName + '/rpt/rptTAssesoldrepuDtl_toEnt',
					method : 'post',
					params : {
						id : keyid
					},
					success : function(result) {
						...
						OSS.setExtValue('saleAssetType', d.saleAssetType);
						OSS.setExtValue('assetTotalVal', d.assetTotalVal);
						OSS.setExtValue('repoPeriodUnit', d.repoPeriodUnit);
						OSS.setExtValue('repoPeriodLen', d.repoPeriodLen);
...
var toUpdate = function() {
	...
	if (formpPanel1.form.isValid()) {
		...
		Ext.Ajax.request({
					url : OSS.getAppName + "/rpt/rptTAssesoldrepuDtl_"
							+ actionId,
					method : 'post',
					params : params,
