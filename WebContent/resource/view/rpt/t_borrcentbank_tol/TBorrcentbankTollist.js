Ext.define('OSS.view.TBorrcentbankTolList', {
	...
	onShbRHClick : function(item, e, eOpts) {
		...
		OSS.setExtValue('tableName', 'T_BorrCentBank_Tol');
		OSS.setExtValue('tPath', 'TBorrcentbankTol');
	},
...
//		window.parent.ContentTablePanelSet('4855916ac2584560a03b5d27e524447d',
//				'向中央银行借款-汇总明细', 'rpt/t_borrcentbank_dtl.jsp?mainId='
...
//				.getSelection();
		...
		window.location.href='t_borrcentbank_dtl.jsp?mainId='+OSS.getExtValue('hidMainId')+ '&'+OSS.paraName+'='+OSS.userId;
	}, 
...
var storeMain = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName
								+ '/rpt/rptTBorrcentbankTol_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName
								+ '/rpt/rptTBorrcentbankDtl_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
				url : OSS.getAppName + "/rpt/rptTBorrcentbankTol_toDel",
				method : 'post',
				params : {
					id : id
				},
...
var InitTBorrcentbankTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TBorrcentbankTolList();
};
