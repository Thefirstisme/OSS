Ext.define('OSS.view.TCentbankfinsavdepoTolList', {
	...
	onShbRHClick : function(item, e, eOpts) {
		...
		OSS.setExtValue('tableName', 'T_CentBankFinSavDepo_Tol');
		OSS.setExtValue('tPath', 'TCentbankfinsavdepoTol');
	},
...
//		window.parent.ContentTablePanelSet('8656d1f67c4349ba9d9dde5200f228c2',
//				'缴存中央银行财政性存款-汇总明细', 'rpt/t_centbankfinsavdepo_dtl.jsp?mainId='
...
//  				.getSelection();
  		...
  		window.location.href='t_centbankfinsavdepo_dtl.jsp?mainId='+OSS.getExtValue('hidMainId')+ '&'+OSS.paraName+'='+OSS.userId;
  	},   
...
var storeMain = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName
								+ '/rpt/rptTCentbankfinsavdepoTol_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName
								+ '/rpt/rptTCentbankfinsavdepoDtl_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
				url : OSS.getAppName + "/rpt/rptTCentbankfinsavdepoTol_toDel",
				method : 'post',
				params : {
					id : id
				},
...
var InitTCentbankfinsavdepoTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TCentbankfinsavdepoTolList();
};
