Ext.define('OSS.view.TCentbankspecdepoTolList', {
	...
	onShbRHClick : function(item, e, eOpts) {
		...
		OSS.setExtValue('tableName', 'T_CentBankSpecDepo_Tol');
		OSS.setExtValue('tPath', 'TCentbankspecdepoTol');
	},
...
//		window.parent.ContentTablePanelSet('317c7c0a1f0f49fa88a07e5dedf17eb8',
//				'存放中央银行特种存款-汇总明细', 'rpt/t_centbankspecdepo_dtl.jsp?mainId='
...
//  				.getSelection();
  		...
  		window.location.href='t_centbankspecdepo_dtl.jsp?mainId='+OSS.getExtValue('hidMainId')+ '&'+OSS.paraName+'='+OSS.userId;
  	},   
...
var storeMain = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName
								+ '/rpt/rptTCentbankspecdepoTol_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
			...
			proxy : new Ext.data.HttpProxy({
						url : OSS.getAppName
								+ '/rpt/rptTCentbankspecdepoDtl_toList',
						reader : {
							type : 'json',
							root : 'data',
							totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
				url : OSS.getAppName + "/rpt/rptTCentbankspecdepoTol_toDel",
				method : 'post',
				params : {
					id : id
				},
...
var InitTCentbankspecdepoTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TCentbankspecdepoTolList();
};
