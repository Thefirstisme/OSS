		...
		.define(
				'OSS.view.TTotareserveTolList',
				{
					...
					onShbRHClick : function(item, e, eOpts) {
						...
						OSS.setExtValue('tableName', 'T_TotaReserve_Tol');
						OSS.setExtValue('tPath', 'TTotareserveTol');
					},
					...
					// window.parent.ContentTablePanelSet('37cc8eb65c6648e0aa00fd03fbb3f143',
					// '各项准备-汇总明细', 'rpt/t_totareserve_dtl.jsp?mainId='
					...
					infoListMore : function(item, e, eOpts) {
						...
						window.location.href = 't_totareserve_dtl.jsp?mainId='
...
var storeMain = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTTotareserveTol_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var logInfoListStore = Ext.create('Ext.data.JsonStore', {
	...
	proxy : new Ext.data.HttpProxy({
		url : OSS.getAppName + '/rpt/rptTTotareserveDtl_toList',
		reader : {
			type : 'json',
			root : 'data',
			totalProperty : 'total'
...
var delData = function(id) {
	...
	myMask.show();
	Ext.Ajax.request({
		url : OSS.getAppName + "/rpt/rptTTotareserveTol_toDel",
		method : 'post',
		params : {
			id : id
		},
...
var InitTTotareserveTolList = function() {
	Ext.tip.QuickTipManager.init();
	new OSS.view.TTotareserveTolList();
};
