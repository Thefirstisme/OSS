var EdTable = function() {
    ...
    function addGridDbClickEvent() {
        $("td.simpleInput").bind("dblclick", function() {
            ...
            var code = OSS.getExtValue("txtReportCode");
            var editCode = OSS.getExtValue("hidEditReportCode");
            var receiveObject = OSS.getExtValue("cmbSearchReceiveObject");
            var submitUnit =  OSS.getExtValue("cmbSearchSubmitUnit");
            //alert(receiveObject);
            var myMask = new Ext.LoadMask(document.body, { msg: '正在校验和缓存数据,请稍等...' });
            myMask.show();
            Ext.Ajax.request({ 
        		url: OSS.getAppName+"/jgb/jgbFormula_saveFormulaFactor",
        		method : 'post', 
        		params : {
        			erc: editCode,
        			rc: code,
        		...
        		success : function(response, options){ 
        			 ...
        			 if(data)
        			 {
        				 if (data.msg == "0") {
        					 ...
        					 OSS.AlertWarning("第一项公式因子只能选择当前编辑的报表单元格");
                         }else{
...
function setButtonExpress(text, type) {
	Ext.Ajax.request({ 
		url: OSS.getAppName+"/jgb/jgbFormula_setButtonExpressText",
		method : 'post', 
...
function setButtonExpressOper(text, type) {
	Ext.Ajax.request({ 
		url: OSS.getAppName+"/jgb/jgbFormula_setButtonExpressTextOper",
		method : 'post', 
		params : {te: text,ty:type},
		success : function(response, options){ 
			 var data = Ext.JSON.decode(response.responseText);  
