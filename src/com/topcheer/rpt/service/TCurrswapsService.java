...
public class TCurrswapsService extends JdbcOracleBaseService{

	public List<?> findTCurrswapsList(String querySql) {
		return (List<?>) getResultPojoList(querySql.toString(),"com.topcheer.rpt.bo.TCurrswaps");
	}

	public List<?> findTCurrswapsList(String querySql,List<Parameter> sqlParameterList) {
		return (List<?>) getResultPojoList(querySql.toString(),sqlParameterList,"com.topcheer.rpt.bo.TCurrswaps");
	}

	public List<?> findTCurrswapsList(String querySql, String classPath) {
		classPath = StringUtils.isEmpty(classPath) ? classPath = "com.topcheer.rpt.bo.TCurrswaps" : classPath;
	...
	*/
	public List<?> getByPage(int startIndex, int endIndex, String filter, String sort){
		...
		String sql = "";
		if (endIndex != -1){
			sql = String.format("SELECT Report_Id || '_' || TO_CHAR (Declare_Dt, 'yyyyMMdd') || '_' || Declare_Seq AS id, if_Need_Change,ifChecked,Report_Id,Declare_Seq,Declare_Dt,Belong_Org_Id,Txn_Type,Fx_Trade_Type,Buy_Currency,Buy_Amt,Buy_Prc,Sell_Currency,Sell_Amt,Sell_Prc,Deliverydate,Opposite_Acctno,Opposite_Acct_Name,Opposite_Bank_Cd,Country_Cd,Domecharge_Area_Cd,Txn_Attri,modified_User,MODIFIED_TIME,CREATE_USER,CREATE_TIME,status FROM (SELECT a.*, ROWNUM rn FROM (SELECT if_Need_Change,ifChecked,Report_Id,Declare_Seq,Declare_Dt,Belong_Org_Id,Txn_Type,Fx_Trade_Type,Buy_Currency,Buy_Amt,Buy_Prc,Sell_Currency,Sell_Amt,Sell_Prc,Deliverydate,Opposite_Acctno,Opposite_Acct_Name,Opposite_Bank_Cd,Country_Cd,Domecharge_Area_Cd,Txn_Attri,modified_User,MODIFIED_TIME,CREATE_USER,CREATE_TIME,status FROM ZMQ_T_CurrSwaps WHERE %2$s order by %1$s) a WHERE ROWNUM <= %4$s) b  WHERE rn > %3$s", sort, filter, startIndex, endIndex);
		}
		else{
			sql = String.format("select if_Need_Change,ifChecked,Report_Id,Declare_Seq,Declare_Dt,Belong_Org_Id,Txn_Type,Fx_Trade_Type,Buy_Currency,Buy_Amt,Buy_Prc,Sell_Currency,Sell_Amt,Sell_Prc,Deliverydate,Opposite_Acctno,Opposite_Acct_Name,Opposite_Bank_Cd,Country_Cd,Domecharge_Area_Cd,Txn_Attri,modified_User,MODIFIED_TIME,CREATE_USER,CREATE_TIME,status from ZMQ_T_CurrSwaps where %2$s order by %1$s", sort, filter);
		}
		return this.findTCurrswapsList(sql);
	}
...
/**
	...
	*/
	public List<?> getByPage(int startIndex, int endIndex, String filter, String sort, String classPath){
		...
		String sql = "";
		if (endIndex != -1){
			sql = String.format("SELECT Report_Id || '_' || TO_CHAR (Declare_Dt, 'yyyyMMdd') || '_' || Declare_Seq AS id, if_Need_Change,ifChecked,Report_Id,Declare_Seq,Declare_Dt,Belong_Org_Id,Txn_Type,Fx_Trade_Type,Buy_Currency,Buy_Amt,Buy_Prc,Sell_Currency,Sell_Amt,Sell_Prc,Deliverydate,Opposite_Acctno,Opposite_Acct_Name,Opposite_Bank_Cd,Country_Cd,Domecharge_Area_Cd,Txn_Attri FROM (SELECT a.*, ROWNUM rn FROM (SELECT if_Need_Change,ifChecked,Report_Id,Declare_Seq,Declare_Dt,Belong_Org_Id,Txn_Type,Fx_Trade_Type,Buy_Currency,Buy_Amt,Buy_Prc,Sell_Currency,Sell_Amt,Sell_Prc,Deliverydate,Opposite_Acctno,Opposite_Acct_Name,Opposite_Bank_Cd,Country_Cd,Domecharge_Area_Cd,Txn_Attri FROM ZMQ_T_CurrSwaps WHERE %2$s order by %1$s) a WHERE ROWNUM <= %4$s) b  WHERE rn > %3$s", sort, filter, startIndex, endIndex);
		}
		else{
          sql = String.format("select if_Need_Change,ifChecked,Report_Id,Declare_Seq,Declare_Dt,Belong_Org_Id,Txn_Type,Fx_Trade_Type,Buy_Currency,Buy_Amt,Buy_Prc,Sell_Currency,Sell_Amt,Sell_Prc,Deliverydate,Opposite_Acctno,Opposite_Acct_Name,Opposite_Bank_Cd,Country_Cd,Domecharge_Area_Cd,Txn_Attri from ZMQ_T_CurrSwaps where %2$s order by %1$s", sort, filter);
		}
		return this.findTCurrswapsList(sql, classPath);
	}
	/**
	* 获得分页 总记录数
	* @param filter
	...
	*/
	public int getPageRowCount(String filter){
		String sql = String.format("select count(*) from ZMQ_T_CurrSwaps where %1$s ", filter);
	...
	* @return
	*/
	public int setTCurrswaps(TCurrswaps ent){
		String sql = "Update ZMQ_T_CurrSwaps Set if_Need_Change=@if_Need_Change, ifChecked=@ifChecked, Belong_Org_Id=@Belong_Org_Id, Txn_Type=@Txn_Type, Fx_Trade_Type=@Fx_Trade_Type, Buy_Currency=@Buy_Currency, Buy_Amt=@Buy_Amt, Buy_Prc=@Buy_Prc, Sell_Currency=@Sell_Currency, Sell_Amt=@Sell_Amt, Sell_Prc=@Sell_Prc, Deliverydate=@Deliverydate, Opposite_Acctno=@Opposite_Acctno, Opposite_Acct_Name=@Opposite_Acct_Name, Opposite_Bank_Cd=@Opposite_Bank_Cd, Country_Cd=@Country_Cd, Domecharge_Area_Cd=@Domecharge_Area_Cd, Txn_Attri=@Txn_Attri,modified_User=zmq_query_ts_user_name(@modifiedUser),modified_time=sysdate Where  Report_Id=@Report_Id and Declare_Dt=@Declare_Dt and Declare_Seq=@Declare_Seq";
		return this.getCount(sql, this.getTCurrswapsParameterList(ent));
	}
	...
	* @return
	*/
	public int delTCurrswaps(String id){
		int result=0;
		if(!id.isEmpty()){
			String[] idList = id.split("_");
			if(idList.length==3){
				String sql = "delete from ZMQ_T_CurrSwaps where Report_Id = @Report_Id and TO_CHAR (Declare_Dt, 'yyyyMMdd') = @Declare_Dt and Declare_Seq=@Declare_Seq";
	...
	* @return
	*/
	public TCurrswaps getTCurrswaps(String id){
		TCurrswaps result = null;
		if(!id.isEmpty()){
			String[] idList = id.split("_");
			if(idList.length==3){
				String sql = "select if_Need_Change,ifChecked,Report_Id,Declare_Seq,Declare_Dt,Belong_Org_Id,Txn_Type,Fx_Trade_Type,Buy_Currency,Buy_Amt,Buy_Prc,Sell_Currency,Sell_Amt,Sell_Prc,Deliverydate,Opposite_Acctno,Opposite_Acct_Name,Opposite_Bank_Cd,Country_Cd,Domecharge_Area_Cd,Txn_Attri,modified_User,MODIFIED_TIME,CREATE_USER,CREATE_TIME,status from ZMQ_T_CurrSwaps WHERE Report_Id = @Report_Id and TO_CHAR (Declare_Dt, 'yyyyMMdd') = @Declare_Dt and Declare_Seq=@Declare_Seq";
				List<Parameter> sqlParameterList = new ArrayList<Parameter>();
				sqlParameterList.add(new Parameter("@Report_Id", DbType.String, idList[0]));
				sqlParameterList.add(new Parameter("@Declare_Dt", DbType.String, idList[1]));
				sqlParameterList.add(new Parameter("@Declare_Seq", DbType.String, idList[2]));
				List<TCurrswaps> list=(List<TCurrswaps>)findTCurrswapsList(sql,sqlParameterList);
	...
	/**
	* 从T_CurrSwaps 实体读取数据，组装为参数集合
	* @param ent
	* @return
	*/
	public List<Parameter> getTCurrswapsParameterList(TCurrswaps ent){
		List<Parameter> sqlParameterList = new ArrayList<Parameter>();
		...
		sqlParameterList.add(new Parameter("@Deliverydate", DbType.Date, ent.getDeliverydate()));
