...
public class TVaultcashTolAction extends BaseAction {
	...
	public void toList() throws Exception {
		...
		List<TVaultcashTol> entList = (List<TVaultcashTol>) tVaultcashTolService
				.getByPage(start, endIndex, filterString, sortString.toString());
		int total = tVaultcashTolService.getPageRowCount(filterString
				.toString());
	...
	public void toAdd() throws Exception {
		TVaultcashTol obj = new TVaultcashTol();
		String oblkeyId = String.format("%1$s_%2$s_%3$s", this
				.getStringParameter("acctId"), DateUtil.date2String(DateUtil
				.string2Date(this.getStringParameter("declareDt"),
						DateUtil.FORMAT_DATE), DateUtil.FORMAT_DATE2), this
				.getStringParameter("detailSeq"));
		TVaultcashTol oldObj = tVaultcashTolService.getTVaultcashTol(oblkeyId);
		if (oldObj != null && oldObj.getAcctId() != null) {
			this.returnJson(false, "数据库已存在主键数据，请更改输入信息！");
			this.log_act.log("添加一条库存现金-汇总: 失败");
		} else {
			...
			int result = tVaultcashTolService.addTVaultcashTol(obj);
			if (result > 0) {
				this.returnJson(true, "添加成功！");
				this.log_act.log("添加一条库存现金-汇总：成功");
			} else {
	...
	public void toDel() throws Exception {
		...
		int result = tVaultcashTolService.delTVaultcashTol(id);
		if (result > 0) {
			this.returnJson(true, "删除成功！");
			this.log_act.log("删除一条库存现金-汇总：成功");
		} else {
			this.returnJson(false, "删除失败！");
			this.log_act.log("删除一条库存现金-汇总：失败");
		}
	}

	public void toEnt() throws Exception {
		String id = this.getStringParameter("id");
		TVaultcashTol ent = tVaultcashTolService.getTVaultcashTol(id);
	...
	public void toEdit() throws Exception {
		...
		if (!seq.isEmpty()) {
			...
			if (result > 0) {
				...
				this.log_act.log("更新一条库存现金-汇总：成功");
			} else {
				this.returnJson(false, "修改失败！");
				this.log_act.log("更新一条库存现金-汇总：失败");
			}
		}
	}
}
