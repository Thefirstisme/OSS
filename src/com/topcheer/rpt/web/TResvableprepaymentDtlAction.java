...
import com.topcheer.rpt.service.TResvableprepaymentDtlService;
import com.topcheer.rpt.service.TResvableprepaymentTolService;
import com.topcheer.rpt.service.TpublishService;

public class TResvableprepaymentDtlAction extends BaseAction{
	 private TpublishService tPublishService;
   private TResvableprepaymentDtlService tResvableprepaymentDtlService;
   private TResvableprepaymentTolService tResvableprepaymentTolService;

     public void settPublishService(TpublishService tPublishService) {
               this.tPublishService = tPublishService;
     }

     public void settResvableprepaymentTolService(TResvableprepaymentTolService tResvableprepaymentTolService) {
               this.tResvableprepaymentTolService = tResvableprepaymentTolService;
     }

	public void settResvableprepaymentDtlService(TResvableprepaymentDtlService tResvableprepaymentDtlService) {
		this.tResvableprepaymentDtlService = tResvableprepaymentDtlService;
	}
	...
	public void toList() throws Exception{
		...
		List<TResvableprepaymentDtl> entList=(List<TResvableprepaymentDtl>)tResvableprepaymentDtlService.getByPage(start, endIndex,filterString, sortString.toString());
		int total = tResvableprepaymentDtlService.getPageRowCount(filterString.toString());
	...
	public void toDown() throws Exception{
		...
		if(total<=65534 && total>0){
			List<?> list=tResvableprepaymentDtlService.getByPage(Integer.parseInt(start), -1,filterString, sortString.toString());
	...
	public void toAdd() throws Exception{
		TResvableprepaymentDtl obj = new TResvableprepaymentDtl();
		...
		TResvableprepaymentDtl oldObj=tResvableprepaymentDtlService.getTResvableprepaymentDtl(oblkeyId);
		...
		obj.setDetailSeq(tPublishService.getDtlSeq("T_ResvablePrepayment_Dtl", obj.getAcctId(), DateUtil.date2String(obj.getDeclareDt(), DateUtil.FORMAT_DATE2)));
		...
		int result=tResvableprepaymentDtlService.addTResvableprepaymentDtl(obj);
		if(result>0){
	...
	public void toDel() throws Exception{
		...
		int result=tResvableprepaymentDtlService.delTResvableprepaymentDtl(id);
		if(result>0){
	...
	public void toEnt() throws Exception{
		String id = this.getStringParameter("id");
		TResvableprepaymentDtl ent = tResvableprepaymentDtlService.getTResvableprepaymentDtl(id);
	...
	public void toEdit() throws Exception{
		...
		if(!seq.isEmpty()){
			TResvableprepaymentDtl obj = tResvableprepaymentDtlService.getTResvableprepaymentDtl(seq);
		obj.setOutInAcctInd(this.getStringParameter("outInAcctInd"));
		obj.setAcctingDt(DateUtil.string2Date(this.getStringParameter("acctingDt"), DateUtil.FORMAT_DATE));
		obj.setOrigTxnDt(DateUtil.string2Date(this.getStringParameter("origTxnDt"), DateUtil.FORMAT_DATE));
		//obj.setBal(this.getStringParameter("bal"));
		...
		obj.setModifiedUser(this.getStringParameter("userId"));
			int result=tResvableprepaymentDtlService.setTResvableprepaymentDtl(obj);
		if(result>0){
	...
	public void toShangBaoDtl() throws Exception{
		String seq=this.getStringParameter("id");
		if(!seq.isEmpty()){
			TResvableprepaymentTol obj = tResvableprepaymentTolService.getTResvableprepaymentTol(seq);
			obj.setIfchecked("1");
			int result=tResvableprepaymentTolService.setTResvableprepaymentTol(obj);
			if(result>0){
