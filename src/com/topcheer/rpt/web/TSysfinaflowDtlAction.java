...
import com.topcheer.rpt.service.TSysfinaflowDtlService;

public class TSysfinaflowDtlAction extends BaseAction{

	private TSysfinaflowDtlService tSysfinaflowDtlService;

	public void settSysfinaflowDtlService(TSysfinaflowDtlService tSysfinaflowDtlService) {
		this.tSysfinaflowDtlService = tSysfinaflowDtlService;
	}
	...
	public void toList() throws Exception{
		...
		List<TSysfinaflowDtl> entList=(List<TSysfinaflowDtl>)tSysfinaflowDtlService.getByPage(start, endIndex,filterString, sortString.toString());
		int total = tSysfinaflowDtlService.getPageRowCount(filterString.toString());
	...
	public void toDown() throws Exception{
		...
		if(total<=65534 && total>0){
			List<?> list=tSysfinaflowDtlService.getByPage(Integer.parseInt(start), -1,filterString, sortString.toString());
			String downFileName="/downfile/"+String.format("系统内资金往来%1$s.xls",DateUtil.timestamp2StringMillis(new Timestamp(System.currentTimeMillis())));
			String fileName = this.getPath() + downFileName;
			FileOutputStream fos = new FileOutputStream( new File(fileName));
			PoiCommon.exportExcel("系统内资金往来",colList.split(";"),list,fos);
			fos.close();
	...
	public void toAdd() throws Exception{
		TSysfinaflowDtl obj = new TSysfinaflowDtl();
		...
		TSysfinaflowDtl oldObj=tSysfinaflowDtlService.getTSysfinaflowDtl(oblkeyId);
		if(oldObj!=null && oldObj.getAcctId()!=null){
		this.returnJson(false, "数据库已存在主键数据，请更改输入信息！");
		this.log_act.log("添加一条应付及暂收款: 失败");
		}else{
		...
		//obj.setDetailSeq(tPublishService.getDtlSeq("T_CorpDepo_Dtl", obj.getAcctId(), DateUtil.date2String(obj.getDeclareDt(), DateUtil.FORMAT_DATE2)));
		...
		int result=tSysfinaflowDtlService.addTSysfinaflowDtl(obj);
		if(result>0){
			this.returnJson(true, "添加成功！");
			this.log_act.log("添加一条系统内资金往来：成功");
		}else{
			this.returnJson(false, "添加失败！");
			this.log_act.log("添加一条系统内资金往来: 失败");
		}
		}
	}
	...
	public void toDel() throws Exception{
		...
		int result=tSysfinaflowDtlService.delTSysfinaflowDtl(id);
		if(result>0){
			this.returnJson(true, "删除成功！");
			this.log_act.log("删除一条系统内资金往来：成功");
		}else{
			this.returnJson(false, "删除失败！");
			this.log_act.log("删除一条系统内资金往来：失败");
		}
	}

	public void toEnt() throws Exception{
		String id = this.getStringParameter("id");
		TSysfinaflowDtl ent = tSysfinaflowDtlService.getTSysfinaflowDtl(id);
	...
	public void toEdit() throws Exception{
		...
		if(!seq.isEmpty()){
			TSysfinaflowDtl obj = tSysfinaflowDtlService.getTSysfinaflowDtl(seq);
		obj.setOutInAcctInd(this.getStringParameter("outInAcctInd"));
		obj.setAcctingDt(DateUtil.string2Date(this.getStringParameter("acctingDt"), DateUtil.FORMAT_DATE));
		obj.setOrigTxnDt(DateUtil.string2Date(this.getStringParameter("origTxnDt"), DateUtil.FORMAT_DATE));
		//obj.setBal(this.getStringParameter("bal"));
		...
		obj.setModifiedUser(this.getStringParameter("userId"));
			int result=tSysfinaflowDtlService.setTSysfinaflowDtl(obj);
		if(result>0){
			this.returnJson(true, "修改成功！");
			this.log_act.log("更新一条系统内资金往来：成功");
		}else{
			this.returnJson(false, "修改失败！");
			this.log_act.log("更新一条系统内资金往来：失败");
		}
		}
	}
}
