...
import com.topcheer.rpt.service.TForextradespotOperDtlService;
import com.topcheer.rpt.service.TForextradespotOperTolService;
import com.topcheer.rpt.service.TpublishService;

public class TForextradespotOperDtlAction extends BaseAction{
	 private TpublishService tPublishService;
   private TForextradespotOperDtlService tForextradespotOperDtlService;
   private TForextradespotOperTolService tForextradespotOperTolService;

     public void settPublishService(TpublishService tPublishService) {
               this.tPublishService = tPublishService;
     }

     public void settForextradespotOperTolService(TForextradespotOperTolService tForextradespotOperTolService) {
               this.tForextradespotOperTolService = tForextradespotOperTolService;
     }

	public void settForextradespotOperDtlService(TForextradespotOperDtlService tForextradespotOperDtlService) {
		this.tForextradespotOperDtlService = tForextradespotOperDtlService;
	}
	...
	public void toList() throws Exception{
		...
		List<TForextradespotOperDtl> entList=(List<TForextradespotOperDtl>)tForextradespotOperDtlService.getByPage(start, endIndex,filterString, sortString.toString());
		int total = tForextradespotOperDtlService.getPageRowCount(filterString.toString());
	...
	public void toDown() throws Exception{
		...
		if(total<=65534 && total>0){
			List<?> list=tForextradespotOperDtlService.getByPage(Integer.parseInt(start), -1,filterString, sortString.toString());
			String downFileName="/downfile/"+String.format("外汇买卖-明细%1$s.xls",DateUtil.timestamp2StringMillis(new Timestamp(System.currentTimeMillis())));
			String fileName = this.getPath() + downFileName;
			FileOutputStream fos = new FileOutputStream( new File(fileName));
			PoiCommon.exportExcel("外汇买卖-明细",colList.split(";"),list,fos);
			fos.close();
	...
	public void toAdd() throws Exception{
		TForextradespotOperDtl obj = new TForextradespotOperDtl();
		...
		TForextradespotOperDtl oldObj=tForextradespotOperDtlService.getTForextradespotOperDtl(oblkeyId);
		if(oldObj!=null && oldObj.getAcctId()!=null){
		this.returnJson(false, "数据库已存在主键数据，请更改输入信息！");
		this.log_act.log("添加一条外汇买卖-明细: 失败");
		}else{
		...
		obj.setDetailSeq(tPublishService.getDtlSeq("T_FOREXTradeSpot_Oper_Dtl", obj.getAcctId(), DateUtil.date2String(obj.getDeclareDt(), DateUtil.FORMAT_DATE2)));
		...
		int result=tForextradespotOperDtlService.addTForextradespotOperDtl(obj);
		if(result>0){
			this.returnJson(true, "添加成功！");
			this.log_act.log("添加一条外汇买卖-明细：成功");
		}else{
	...
	public void toDel() throws Exception{
		...
		int result=tForextradespotOperDtlService.delTForextradespotOperDtl(id);
		if(result>0){
			this.returnJson(true, "删除成功！");
			this.log_act.log("删除一条外汇买卖-明细：成功");
		}else{
			this.returnJson(false, "删除失败！");
			this.log_act.log("删除一条外汇买卖-明细：失败");
		}
	}

	public void toEnt() throws Exception{
		String id = this.getStringParameter("id");
		TForextradespotOperDtl ent = tForextradespotOperDtlService.getTForextradespotOperDtl(id);
	...
	public void toEdit() throws Exception{
		...
		if(!seq.isEmpty()){
			TForextradespotOperDtl obj = tForextradespotOperDtlService.getTForextradespotOperDtl(seq);
		obj.setOutInAcctInd(this.getStringParameter("outInAcctInd"));
		obj.setAcctingDt(DateUtil.string2Date(this.getStringParameter("acctingDt"), DateUtil.FORMAT_DATE));
		obj.setOrigTxnDt(DateUtil.string2Date(this.getStringParameter("origTxnDt"), DateUtil.FORMAT_DATE));
		obj.setFxTradeType(this.getStringParameter("fxTradeType"));
		...
		obj.setModifiedUser(this.getStringParameter("userId"));
			int result=tForextradespotOperDtlService.setTForextradespotOperDtl(obj);
		if(result>0){
			this.returnJson(true, "修改成功！");
			this.log_act.log("更新一条外汇买卖-明细：成功");
		}else{
			this.returnJson(false, "修改失败！");
			this.log_act.log("更新一条外汇买卖-明细：失败");
		}
		}
	}
	/**
	...
	public void toShangBaoDtl() throws Exception{
		String seq=this.getStringParameter("id");
		if(!seq.isEmpty()){
			TForextradespotOperTol obj = tForextradespotOperTolService.getTForextradespotOperTol(seq);
			obj.setIfchecked("1");
			int result=tForextradespotOperTolService.setTForextradespotOperTol(obj);
			if(result>0){
