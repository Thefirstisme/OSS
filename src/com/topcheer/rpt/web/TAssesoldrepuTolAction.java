...
public class TAssesoldrepuTolAction extends BaseAction{
   ...
   private TAssesoldrepuDtlService tAssesoldrepuDtlService;
	...
	public void toList() throws Exception{
		...
		List<TAssesoldrepuTol> entList=(List<TAssesoldrepuTol>)tAssesoldrepuTolService.getByPage(start, endIndex,filterString, sortString.toString());
		int total = tAssesoldrepuTolService.getPageRowCount(filterString.toString());
	...
	public void toDown() throws Exception{
		...
		if(total<=65534 && total>0){
			List<?> list=tAssesoldrepuTolService.getByPage(Integer.parseInt(start), -1,filterString, sortString.toString());
			String downFileName="/downfile/"+String.format("卖出回购资产-汇总%1$s.xls",DateUtil.timestamp2StringMillis(new Timestamp(System.currentTimeMillis())));
			String fileName = this.getPath() + downFileName;
			FileOutputStream fos = new FileOutputStream( new File(fileName));
			PoiCommon.exportExcel("卖出回购资产-汇总",colList.split(";"),list,fos);
			fos.close();
	...
	public void toAdd() throws Exception{
		TAssesoldrepuTol obj = new TAssesoldrepuTol();
		...
		TAssesoldrepuTol oldObj=tAssesoldrepuTolService.getTAssesoldrepuTol(oblkeyId);
		if(oldObj!=null && oldObj.getAcctId()!=null){
		this.returnJson(false, "数据库已存在主键数据，请更改输入信息！");
		this.log_act.log("添加一条卖出回购资产-汇总: 失败");
		}else{
		...
		int result=tAssesoldrepuTolService.addTAssesoldrepuTol(obj);
		if(result>0){
			this.returnJson(true, "添加成功！");
			this.log_act.log("添加一条卖出回购资产-汇总：成功");
		}else{
	...
	public void toDel() throws Exception{
		...
		int result=tAssesoldrepuTolService.delTAssesoldrepuTol(id);
		if(result>0){
			this.returnJson(true, "删除成功！");
			this.log_act.log("删除一条卖出回购资产-汇总：成功");
		}else{
			this.returnJson(false, "删除失败！");
			this.log_act.log("删除一条卖出回购资产-汇总：失败");
		}
	}

	public void toEnt() throws Exception{
		String id = this.getStringParameter("id");
		TAssesoldrepuTol ent = tAssesoldrepuTolService.getTAssesoldrepuTol(id);
	...
	public void toEdit() throws Exception{
		...
		if(result>0){
			...
			this.log_act.log("更新一条卖出回购资产-汇总：成功");
		}else{
			this.returnJson(false, "修改失败！");
			this.log_act.log("更新一条卖出回购资产-汇总：失败");
		}
		}
	}
}
