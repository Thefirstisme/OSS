...
import com.topcheer.rpt.service.TBuybackassetacquiredDtlService;
import com.topcheer.rpt.service.TBuybackassetacquiredTolService;
import com.topcheer.rpt.service.TpublishService;

public class TBuybackassetacquiredDtlAction extends BaseAction{
	 private TpublishService tPublishService;
   private TBuybackassetacquiredDtlService tBuybackassetacquiredDtlService;
   private TBuybackassetacquiredTolService tBuybackassetacquiredTolService;

     public void settPublishService(TpublishService tPublishService) {
               this.tPublishService = tPublishService;
     }

     public void settBuybackassetacquiredTolService(TBuybackassetacquiredTolService tBuybackassetacquiredTolService) {
               this.tBuybackassetacquiredTolService = tBuybackassetacquiredTolService;
     }

	public void settBuybackassetacquiredDtlService(TBuybackassetacquiredDtlService tBuybackassetacquiredDtlService) {
		this.tBuybackassetacquiredDtlService = tBuybackassetacquiredDtlService;
	}
	...
	public void toList() throws Exception{
		...
		List<TBuybackassetacquiredDtl> entList=(List<TBuybackassetacquiredDtl>)tBuybackassetacquiredDtlService.getByPage(start, endIndex,filterString, sortString.toString());
		int total = tBuybackassetacquiredDtlService.getPageRowCount(filterString.toString());
	...
	public void toDown() throws Exception{
		...
		if(total<=65534 && total>0){
			List<?> list=tBuybackassetacquiredDtlService.getByPage(Integer.parseInt(start), -1,filterString, sortString.toString());
			String downFileName="/downfile/"+String.format("买入返售资产-明细%1$s.xls",DateUtil.timestamp2StringMillis(new Timestamp(System.currentTimeMillis())));
			String fileName = this.getPath() + downFileName;
			FileOutputStream fos = new FileOutputStream( new File(fileName));
			PoiCommon.exportExcel("买入返售资产-明细",colList.split(";"),list,fos);
			fos.close();
	...
	public void toAdd() throws Exception{
		TBuybackassetacquiredDtl obj = new TBuybackassetacquiredDtl();
		...
		TBuybackassetacquiredDtl oldObj=tBuybackassetacquiredDtlService.getTBuybackassetacquiredDtl(oblkeyId);
		if(oldObj!=null && oldObj.getAcctId()!=null){
		this.returnJson(false, "数据库已存在主键数据，请更改输入信息！");
		this.log_act.log("添加一条买入返售资产-明细: 失败");
		}else{
		...
		obj.setDetailSeq(tPublishService.getDtlSeq("T_BuyBackAssetAcquired_Dtl", obj.getAcctId(), DateUtil.date2String(obj.getDeclareDt(), DateUtil.FORMAT_DATE2)));
		...
		obj.setOppositeCustAttri(this.getStringParameter("oppositeCustAttri"));
		...
		obj.setBuyAssetType(this.getStringParameter("buyAssetType"));
		//obj.setOrigAssetTotalVal(this.getStringParameter("origAssetTotalVal"));
		String origAssetTotalVal = this.getStringParameter("origAssetTotalVal");
		if(StringUtils.isNotEmpty(origAssetTotalVal)){
			obj.setOrigAssetTotalVal(new BigDecimal(origAssetTotalVal));
		}
		obj.setOrigPeriodUnit(this.getStringParameter("origPeriodUnit"));
		obj.setOrigPeriodLen(this.getStringParameter("origPeriodLen"));
		obj.setTxnAttri(this.getStringParameter("txnAttri"));
		obj.setIfNeedChange("2");
		obj.setCreateUser(this.getStringParameter("userId")) ;
		int result=tBuybackassetacquiredDtlService.addTBuybackassetacquiredDtl(obj);
		if(result>0){
			this.returnJson(true, "添加成功！");
			this.log_act.log("添加一条买入返售资产-明细：成功");
		}else{
	...
	public void toDel() throws Exception{
		...
		int result=tBuybackassetacquiredDtlService.delTBuybackassetacquiredDtl(id);
		if(result>0){
			this.returnJson(true, "删除成功！");
			this.log_act.log("删除一条买入返售资产-明细：成功");
		}else{
			this.returnJson(false, "删除失败！");
			this.log_act.log("删除一条买入返售资产-明细：失败");
		}
	}

	public void toEnt() throws Exception{
		String id = this.getStringParameter("id");
		TBuybackassetacquiredDtl ent = tBuybackassetacquiredDtlService.getTBuybackassetacquiredDtl(id);
	...
	public void toEdit() throws Exception{
		...
		if(!seq.isEmpty()){
			TBuybackassetacquiredDtl obj = tBuybackassetacquiredDtlService.getTBuybackassetacquiredDtl(seq);
		obj.setOutInAcctInd(this.getStringParameter("outInAcctInd"));
		obj.setAcctingDt(DateUtil.string2Date(this.getStringParameter("acctingDt"), DateUtil.FORMAT_DATE));
		obj.setOrigTxnDt(DateUtil.string2Date(this.getStringParameter("origTxnDt"), DateUtil.FORMAT_DATE));
		obj.setIntStartDt(DateUtil.string2Date(this.getStringParameter("intStartDt"), DateUtil.FORMAT_DATE));
		...
		obj.setModifiedUser(this.getStringParameter("userId"));
			int result=tBuybackassetacquiredDtlService.setTBuybackassetacquiredDtl(obj);
		if(result>0){
			this.returnJson(true, "修改成功！");
			this.log_act.log("更新一条买入返售资产-明细：成功");
		}else{
			this.returnJson(false, "修改失败！");
			this.log_act.log("更新一条买入返售资产-明细：失败");
		}
		}
	}
	...
	public void toShangBaoDtl() throws Exception{
		String seq=this.getStringParameter("id");
		if(!seq.isEmpty()){
			TBuybackassetacquiredTol obj = tBuybackassetacquiredTolService.getTBuybackassetacquiredTol(seq);
			obj.setIfchecked("1");
			int result=tBuybackassetacquiredTolService.setTBuybackassetacquiredTol(obj);
			if(result>0){
