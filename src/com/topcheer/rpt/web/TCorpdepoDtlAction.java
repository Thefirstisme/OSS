...
public class TCorpdepoDtlAction extends BaseAction{
   ...
   private TCorpdepoTolService tCorpdepoTolService;
	...
	public void toList() throws Exception{
		...
		List<TCorpdepoDtl> entList=(List<TCorpdepoDtl>)tCorpdepoDtlService.getByPage(start, endIndex,filterString, sortString.toString());
		int total = tCorpdepoDtlService.getPageRowCount(filterString.toString());
	...
	public void toDown() throws Exception{
		...
		if(total<=65534 && total>0){
			List<?> list=tCorpdepoDtlService.getByPage(Integer.parseInt(start), -1,filterString, sortString.toString());
			String downFileName="/downfile/"+String.format("单位存款-明细%1$s.xls",DateUtil.timestamp2StringMillis(new Timestamp(System.currentTimeMillis())));
			String fileName = this.getPath() + downFileName;
			FileOutputStream fos = new FileOutputStream( new File(fileName));
			PoiCommon.exportExcel("单位存款-明细",colList.split(";"),list,fos);
			fos.close();
	...
	public void toAdd() throws Exception{
		TCorpdepoDtl obj = new TCorpdepoDtl();
		...
		TCorpdepoDtl oldObj=tCorpdepoDtlService.getTCorpdepoDtl(oblkeyId);
		if(oldObj!=null && oldObj.getAcctId()!=null){
		this.returnJson(false, "数据库已存在主键数据，请更改输入信息！");
		this.log_act.log("添加一条单位存款-明细: 失败");
		}else{
		...
		int result=tCorpdepoDtlService.addTCorpdepoDtl(obj);
		if(result>0){
			this.returnJson(true, "添加成功！");
			this.log_act.log("添加一条单位存款-明细：成功");
		}else{
	...
	public void toDel() throws Exception{
		...
		int result=tCorpdepoDtlService.delTCorpdepoDtl(id);
		if(result>0){
			this.returnJson(true, "删除成功！");
			this.log_act.log("删除一条单位存款-明细：成功");
		}else{
			this.returnJson(false, "删除失败！");
			this.log_act.log("删除一条单位存款-明细：失败");
		}
	}

	public void toEnt() throws Exception{
		String id = this.getStringParameter("id");
		TCorpdepoDtl ent = tCorpdepoDtlService.getTCorpdepoDtl(id);
	...
	public void toEdit() throws Exception{
		...
		if(!seq.isEmpty()){
			TCorpdepoDtl obj = tCorpdepoDtlService.getTCorpdepoDtl(seq);
		obj.setOutInAcctInd(this.getStringParameter("outInAcctInd"));
		obj.setAcctingDt(DateUtil.string2Date(this.getStringParameter("acctingDt"), DateUtil.FORMAT_DATE));
		obj.setOrigTxnDt(DateUtil.string2Date(this.getStringParameter("origTxnDt"), DateUtil.FORMAT_DATE));
		//obj.setBal(this.getStringParameter("bal"));
		...
		obj.setIfNeedChange("2");
			int result=tCorpdepoDtlService.setTCorpdepoDtl(obj);
		if(result>0){
			this.returnJson(true, "修改成功！");
			this.log_act.log("更新一条单位存款-明细：成功");
		}else{
			this.returnJson(false, "修改失败！");
			this.log_act.log("更新一条单位存款-明细：失败");
		}
		}
	}
	/**
