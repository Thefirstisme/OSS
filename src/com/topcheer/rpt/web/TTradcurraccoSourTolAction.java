...
public class TTradcurraccoSourTolAction extends BaseAction{
   ...
   private TTradcurraccoSourDtlService tTradcurraccoSourDtlService;
	...
	public void toList() throws Exception{
		...
		List<TTradcurraccoSourTol> entList=(List<TTradcurraccoSourTol>)tTradcurraccoSourTolService.getByPage(start, endIndex,filterString, sortString.toString());
		int total = tTradcurraccoSourTolService.getPageRowCount(filterString.toString());
	...
	public void toDown() throws Exception{
		...
		if(total<=65534 && total>0){
			List<?> list=tTradcurraccoSourTolService.getByPage(Integer.parseInt(start), -1,filterString, sortString.toString());
			String downFileName="/downfile/"+String.format("同业往来（来源方）-汇总%1$s.xls",DateUtil.timestamp2StringMillis(new Timestamp(System.currentTimeMillis())));
			String fileName = this.getPath() + downFileName;
			FileOutputStream fos = new FileOutputStream( new File(fileName));
			PoiCommon.exportExcel("同业往来（来源方）-汇总",colList.split(";"),list,fos);
			fos.close();
	...
	public void toAdd() throws Exception{
		TTradcurraccoSourTol obj = new TTradcurraccoSourTol();
		String oblkeyId=String.format("%1$s_%2$s", this.getStringParameter("acctId"),DateUtil.date2String(DateUtil.string2Date(this.getStringParameter("declareDt"), DateUtil.FORMAT_DATE), DateUtil.FORMAT_DATE2), this.getStringParameter("currencyCd"));
		TTradcurraccoSourTol oldObj=tTradcurraccoSourTolService.getTTradcurraccoSourTol(oblkeyId);
		if(oldObj!=null && oldObj.getAcctId()!=null){
		this.returnJson(false, "数据库已存在主键数据，请更改输入信息！");
		this.log_act.log("添加一条同业往来（来源方）-汇总: 失败");
		}else{
		...
		int result=tTradcurraccoSourTolService.addTTradcurraccoSourTol(obj);
		if(result>0){
			this.returnJson(true, "添加成功！");
			this.log_act.log("添加一条同业往来（来源方）-汇总：成功");
		}else{
	...
	public void toDel() throws Exception{
		...
		int result=tTradcurraccoSourTolService.delTTradcurraccoSourTol(id);
		if(result>0){
			this.returnJson(true, "删除成功！");
			this.log_act.log("删除一条同业往来（来源方）-汇总：成功");
		}else{
			this.returnJson(false, "删除失败！");
			this.log_act.log("删除一条同业往来（来源方）-汇总：失败");
		}
	}

	public void toEnt() throws Exception{
		String id = this.getStringParameter("id");
		TTradcurraccoSourTol ent = tTradcurraccoSourTolService.getTTradcurraccoSourTol(id);
	...
	public void toEdit() throws Exception{
		...
		if(result>0){
			...
			this.log_act.log("更新一条同业往来（来源方）-汇总：成功");
		}else{
			this.returnJson(false, "修改失败！");
			this.log_act.log("更新一条同业往来（来源方）-汇总：失败");
		}
		}
	}
}
