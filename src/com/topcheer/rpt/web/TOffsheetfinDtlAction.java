...
public class TOffsheetfinDtlAction extends BaseAction{
   ...
   private TOffsheetfinTolService tOffsheetfinTolService;
	...
	public void toList() throws Exception{
		...
		List<TOffsheetfinDtl> entList=(List<TOffsheetfinDtl>)tOffsheetfinDtlService.getByPage(start, endIndex,filterString, sortString.toString());
		int total = tOffsheetfinDtlService.getPageRowCount(filterString.toString());
	...
	public void toDown() throws Exception{
		...
		if(total<=65534 && total>0){
			List<?> list=tOffsheetfinDtlService.getByPage(Integer.parseInt(start), -1,filterString, sortString.toString());
			String downFileName="/downfile/"+String.format("表外理财-明细%1$s.xls",DateUtil.timestamp2StringMillis(new Timestamp(System.currentTimeMillis())));
			String fileName = this.getPath() + downFileName;
			FileOutputStream fos = new FileOutputStream( new File(fileName));
			PoiCommon.exportExcel("表外理财-明细",colList.split(";"),list,fos);
			fos.close();
	...
	public void toAdd() throws Exception{
		TOffsheetfinDtl obj = new TOffsheetfinDtl();
		...
		TOffsheetfinDtl oldObj=tOffsheetfinDtlService.getTOffsheetfinDtl(oblkeyId);
		if(oldObj!=null && oldObj.getAcctId()!=null){
		this.returnJson(false, "数据库已存在主键数据，请更改输入信息！");
		this.log_act.log("添加一条表外理财-明细: 失败");
		}else{
		...
		obj.setDetailSeq(tPublishService.getDtlSeq("T_OffsheetFin_Dtl", obj.getAcctId(), DateUtil.date2String(obj.getDeclareDt(), DateUtil.FORMAT_DATE2)));
		...
		//obj.setBal(Double.parseDouble(this.getStringParameter("bal")));
		...
		//obj.setRate(Double.parseDouble(this.getStringParameter("rate")));
		...
		int result=tOffsheetfinDtlService.addTOffsheetfinDtl(obj);
		if(result>0){
			this.returnJson(true, "添加成功！");
			this.log_act.log("添加一条表外理财-明细：成功");
		}else{
	...
	public void toDel() throws Exception{
		...
		int result=tOffsheetfinDtlService.delTOffsheetfinDtl(id);
		if(result>0){
			this.returnJson(true, "删除成功！");
			this.log_act.log("删除一条表外理财-明细：成功");
		}else{
			this.returnJson(false, "删除失败！");
			this.log_act.log("删除一条表外理财-明细：失败");
		}
	}

	public void toEnt() throws Exception{
		String id = this.getStringParameter("id");
		TOffsheetfinDtl ent = tOffsheetfinDtlService.getTOffsheetfinDtl(id);
	...
	public void toEdit() throws Exception{
		...
		if(!seq.isEmpty()){
			TOffsheetfinDtl obj = tOffsheetfinDtlService.getTOffsheetfinDtl(seq);
		obj.setOutInAcctInd(this.getStringParameter("outInAcctInd"));
		obj.setAcctDt(DateUtil.string2Date(this.getStringParameter("acctDt"), DateUtil.FORMAT_DATE));
		obj.setOrigTxnDt(DateUtil.string2Date(this.getStringParameter("origTxnDt"), DateUtil.FORMAT_DATE));
		//obj.setBal(Double.parseDouble(this.getStringParameter("bal")));
		...
		obj.setModifiedUser(this.getStringParameter("userId"));
			int result=tOffsheetfinDtlService.setTOffsheetfinDtl(obj);
		if(result>0){
			this.returnJson(true, "修改成功！");
			this.log_act.log("更新一条表外理财-明细：成功");
		}else{
			this.returnJson(false, "修改失败！");
			this.log_act.log("更新一条表外理财-明细：失败");
		}
		}
	}
	/**
