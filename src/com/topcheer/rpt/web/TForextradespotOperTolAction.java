...
public class TForextradespotOperTolAction extends BaseAction{
   ...
   private TForextradespotOperDtlService tForextradespotOperDtlService;
	...
	public void toList() throws Exception{
		...
		List<TForextradespotOperTol> entList=(List<TForextradespotOperTol>)tForextradespotOperTolService.getByPage(start, endIndex,filterString, sortString.toString());
		int total = tForextradespotOperTolService.getPageRowCount(filterString.toString());
	...
	public void toDown() throws Exception{
		...
		if(total<=65534 && total>0){
			List<?> list=tForextradespotOperTolService.getByPage(Integer.parseInt(start), -1,filterString, sortString.toString());
			String downFileName="/downfile/"+String.format("外汇买卖-汇总%1$s.xls",DateUtil.timestamp2StringMillis(new Timestamp(System.currentTimeMillis())));
			String fileName = this.getPath() + downFileName;
			FileOutputStream fos = new FileOutputStream( new File(fileName));
			PoiCommon.exportExcel("外汇买卖-汇总",colList.split(";"),list,fos);
			fos.close();
	...
	public void toAdd() throws Exception{
		TForextradespotOperTol obj = new TForextradespotOperTol();
		...
		TForextradespotOperTol oldObj=tForextradespotOperTolService.getTForextradespotOperTol(oblkeyId);
		if(oldObj!=null && oldObj.getAcctId()!=null){
		this.returnJson(false, "数据库已存在主键数据，请更改输入信息！");
		this.log_act.log("添加一条外汇买卖-汇总: 失败");
		}else{
		...
		int result=tForextradespotOperTolService.addTForextradespotOperTol(obj);
		if(result>0){
			this.returnJson(true, "添加成功！");
			this.log_act.log("添加一条外汇买卖-汇总：成功");
		}else{
	...
	public void toDel() throws Exception{
		...
		int result=tForextradespotOperTolService.delTForextradespotOperTol(id);
		if(result>0){
			this.returnJson(true, "删除成功！");
			this.log_act.log("删除一条外汇买卖-汇总：成功");
		}else{
			this.returnJson(false, "删除失败！");
			this.log_act.log("删除一条外汇买卖-汇总：失败");
		}
	}

	public void toEnt() throws Exception{
		String id = this.getStringParameter("id");
		TForextradespotOperTol ent = tForextradespotOperTolService.getTForextradespotOperTol(id);
	...
	public void toEdit() throws Exception{
		...
		if(result>0){
			...
			this.log_act.log("更新一条外汇买卖-汇总：成功");
		}else{
			this.returnJson(false, "修改失败！");
			this.log_act.log("更新一条外汇买卖-汇总：失败");
		}
		}
	}
}
