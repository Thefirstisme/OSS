...
import com.topcheer.rpt.service.TBankaccepbillPayService;
import com.topcheer.rpt.service.TpublishService;

public class TBankaccepbillPayAction extends BaseAction{

	private TBankaccepbillPayService tBankaccepbillPayService;
	...
	public void settBankaccepbillPayService(TBankaccepbillPayService tBankaccepbillPayService) {
		this.tBankaccepbillPayService = tBankaccepbillPayService;
	}
	...
	public void toList() throws Exception{
		...
		List<TBankaccepbillPay> entList=(List<TBankaccepbillPay>)tBankaccepbillPayService.getByPage(start, endIndex,filterString, sortString.toString());
		int total = tBankaccepbillPayService.getPageRowCount(filterString.toString());
	...
	public void toDown() throws Exception{
		...
		if(total<=65534 && total>0){
			List<?> list=tBankaccepbillPayService.getByPage(Integer.parseInt(start), -1,filterString, sortString.toString());
			String downFileName="/downfile/"+String.format("应付银行承兑汇票%1$s.xls",DateUtil.timestamp2StringMillis(new Timestamp(System.currentTimeMillis())));
			String fileName = this.getPath() + downFileName;
			FileOutputStream fos = new FileOutputStream( new File(fileName));
			PoiCommon.exportExcel("应付银行承兑汇票",colList.split(";"),list,fos);
			fos.close();
	...
	public void toAdd() throws Exception{
		TBankaccepbillPay obj = new TBankaccepbillPay();
		...
		TBankaccepbillPay oldObj=tBankaccepbillPayService.getTBankaccepbillPay(oblkeyId);
		if(oldObj!=null && oldObj.getReportId()!=null){
		this.returnJson(false, "数据库已存在主键数据，请更改输入信息！");
		this.log_act.log("添加一条应付银行承兑汇票: 失败");
		}else{
		...
		obj.setDeclareSeq(tPublishService.getDeclareSeq("T_BankAccepBill_Pay",obj.getReportId(),DateUtil.date2String(obj.getDeclareDt(), DateUtil.FORMAT_DATE2)));
		obj.setBelongOrgId(this.getStringParameter("belongOrgId"));
		obj.setAcptDt(DateUtil.string2Date(this.getStringParameter("acptDt"), DateUtil.FORMAT_DATE));
		...
		obj.setCorpAcctId(this.getStringParameter("corpAcctId"));
		obj.setCorpNm(this.getStringParameter("corpNm"));
		obj.setCorpId(this.getStringParameter("corpId"));
		...
		int result=tBankaccepbillPayService.addTBankaccepbillPay(obj);
		if(result>0){
			this.returnJson(true, "添加成功！");
			this.log_act.log("添加一条应付银行承兑汇票：成功");
		}else{
	...
	public void toDel() throws Exception{
		...
		int result=tBankaccepbillPayService.delTBankaccepbillPay(id);
		if(result>0){
			this.returnJson(true, "删除成功！");
			this.log_act.log("删除一条应付银行承兑汇票：成功");
		}else{
			this.returnJson(false, "删除失败！");
			this.log_act.log("删除一条应付银行承兑汇票：失败");
		}
	}

	public void toEnt() throws Exception{
		String id = this.getStringParameter("id");
		TBankaccepbillPay ent = tBankaccepbillPayService.getTBankaccepbillPay(id);
	...
	public void toEdit() throws Exception{
		...
		if(!seq.isEmpty()){
			TBankaccepbillPay obj = tBankaccepbillPayService.getTBankaccepbillPay(seq);
		obj.setReportId(this.getStringParameter("reportId"));
		obj.setBelongOrgId(this.getStringParameter("belongOrgId"));
		obj.setAcptDt(DateUtil.string2Date(this.getStringParameter("acptDt"), DateUtil.FORMAT_DATE));
		obj.setAcptMatureDt(DateUtil.string2Date(this.getStringParameter("acptMatureDt"), DateUtil.FORMAT_DATE));
		...
		obj.setModifiedUser(this.getStringParameter("userId"));
			int result=tBankaccepbillPayService.setTBankaccepbillPay(obj);
		if(result>0){
			this.returnJson(true, "修改成功！");
			this.log_act.log("更新一条应付银行承兑汇票：成功");
		}else{
			this.returnJson(false, "修改失败！");
			this.log_act.log("更新一条应付银行承兑汇票：失败");
		}
		}
	}
}
