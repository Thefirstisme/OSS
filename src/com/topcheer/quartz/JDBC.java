...
public class JDBC {
	 ...
	 * @throws Exception
	 */
	public synchronized static TaskLogBean insert(String sql, HashMap map)
			throws Exception {
		Connection con = getConnection();
		String sql_ = "SELECT MAX(TASK_LOG_ID)+1 FROM ZMQ_TASK_LOG"; // 获取最大的值+1
																	// 作为主键，这里方法比较土。
		...
		ps.executeUpdate();
		sql_ = "SELECT * FROM ZMQ_TASK_LOG WHERE TASK_LOG_ID=:TASK_LOG_ID";
	 ...
	 * @throws Exception
	 */
	public static JobTaskBean[] getJobTaskBean(String sql, HashMap map)
			throws Exception {
		Connection con = getConnection();
		...
		rs.close();
		con.close();
		return list.size() == 0 ? null : (JobTaskBean[]) list
				.toArray(new JobTaskBean[0]);
	}
	 ...
	 * @throws Exception
	 */
	public static TaskLogBean[] getTaskLogBean(Connection con, String sql,
		...
		rs.close();
		con.close();
		return list.size() == 0 ? null : (TaskLogBean[]) list
				.toArray(new TaskLogBean[0]);
	}
	 ...
	 * @throws SQLException
	 */
	public static PreparedStatement execStatementValues(Connection con,
			String sql, HashMap map) throws Exception {
		...
		for (int i = 1; i < temp.length; i++) {
			try {
				clumBandNameList
						.add(temp[i].substring(0, temp[i].indexOf(" ")));
		...
		for (int i = 0; i < clumBandNameList.size(); i++) {
			...
			try {
				...
				throw new Exception("变量：" + clumBandNameList.get(i)
						+ " 没有对应绑定的值");
			}

			String type = value.getClass().getName()
					.substring(value.getClass().getName().lastIndexOf(".")); // 获取绑定的类型
			index = i + 1; // 绑定的索引
